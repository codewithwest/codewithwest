        ...
        <div className="relative mx-auto max-w-3xl">
          ...
          <div className="relative flex flex-col gap-12">
            {experiences.map((exp, index) => (
              <div key={exp.role} className="relative flex items-start">
                ...
                >
                  <div className="w-1/2 text-left rtl:text-right">
                    {index % 2 === 1 && (
                      <>
                        <h3 className="font-headline text-lg font-bold">{exp.role}</h3>
                        <p className="font-medium text-primary">{exp.company}</p>
                        <p className="mt-1 text-sm text-muted-foreground">{exp.duration}</p>

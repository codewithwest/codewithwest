...
const JourneyListItem = (journeyItem: journeyItemType, index: number) => {
  ...
  return (
    <li className="mb-10 ms-4" key={index}>
      <div
        className="absolute w-3 h-3 bg-gray-200 rounded-full mt-1.5 -start-1.5 border 
      ...
      <h3 className="text-lg font-semibold dark:text-current">
        {journeyItem.title}
      </h3>
      ...
      {journeyItem.link == "#" ? null : (
        <a
          ...
          className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 
            bg-white border border-current rounded-lg hover:bg-gray-100 hover:text-blue-700 
